...
{	
	function __construct(){
		...
		$this->load->library(array('form_validation','authentication','pagination','template','asset', 'session'));
		$this->load->helper(array('form', 'url', 'system', 'date'));
...
abstract class REST_Controller extends MX_Controller {
    ...
    public function _remap($object_called, $arguments)
    {
        ...
        catch (Exception $ex)
        {
            ...
            $this->response(array(
                    $this->config->item('rest_status_field_name') => FALSE,
                    $this->config->item('rest_message_field_name') => array(
