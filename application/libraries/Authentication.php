...
class Authentication
{
	...
	function logout()
	{
		...
		$this->ci->session->unset_userdata(array('any_user_group_exist' , 'user_id' , 'user_email' , 'user_name' , 'user_real_name' , 'user_group' , 'is_logged_in', 'menu_privilege', 'action_privilege','user_role_id','user_unit_kode','user_unit_id'));
	}
	...
	function menu_privilege($qry)
	{
		...
		if ( !is_null( $qry ) ) {
			...
			foreach ($qry as $row)
			{
				$data_menu[$row->MenuId] = array('MenuId'=> $row->MenuId, 'MenuName'=> $row->MenuName, 'MenuParentId'=> $row->MenuParentId, 'MenuModule'=> $row->MenuModule, 'MenuIcon'=> $row->MenuIcon, 'MenuDescription'=> $row->MenuDescription, 'MenuIconClass'=> $row->MenuIconClass, 'MenuOrder'=> $row->MenuOrder,);
		...
		if(isset($data[$parrent])){ 
			foreach($data[$parrent] as $value){				
				...
				if( $child ){
					$str .= '
							<li class="hover">
								<a href="'. site_url( $value['MenuModule']  ) .'" class="dropdown-toggle">
									<i class="menu-icon fa '. $value['MenuIconClass'] .'"></i>
					...
					$str .= '<li class="hover">
								<a href="'. site_url( $value['MenuModule'] ) .'">
									<i class="menu-icon fa '. $value['MenuIconClass'] .'"></i>
									' . $value['MenuName'] . '
								</a>
								<b class="arrow"></b>
